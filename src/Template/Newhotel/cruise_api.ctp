...
$session_search = $this->request->session()->read('cruise.search');
    ?>
     <?php 
        if($min_price > 0 ){
         ?>
        ...
        <div class="header-trans pb-3">
            <div class="row">
                <div class="col-md-8">
                    <h2 data-bind="">
                    <?php
                       echo ($session_search['destination'] == '1')? 'HaLong' : 'Mekong';
                  //   echo $session_search['destination'] ?> ,
                     <?php echo $session_search['formm'] ?> » 
                      <?php echo $session_search['to'] ?>
        ...
        <div class="khung-property">
            <div class="row card-color-1 m-0 px-2" id="<?php echo  ($i%2) ? 'color-the' : ''  ?>" style="height: 80px;font-size: 14px;">
                <div class="col-md-1 fix-img-proprety p-0" style="margin: auto;">
                    ...
                    <?php echo $this->Html->image('/upload/cruisedrive/'.$transferdrive_view[$key]['image'], ['alt' => 'hinhanh','class'=>'','style'=>'width: 100%;']);?>
